/*
 * @Author: YangChao 
 * @Date: 2020-02-16 18:13:34 
 * @Last Modified by: YangChao
 * @Last Modified time: 2020-02-16 20:22:13
 */
import {temptureTestData} from './data.js';
class temputureMain{
    constructor(viewer){
        this.viewer = viewer;
        $("#tempId").fadeIn();

        var datasource = Cesium.GeoJsonDataSource.load('./bounds.geojson', {
        ...
        $("#closeTemp").click(function(){
            viewer.imageryLayers.remove(this.temptureLayer,true);
            //console.log(promise)
    ...
    loadkriging(viewer) {
        ...
        this.temptureLayer = viewer.imageryLayers.addImageryProvider(new Cesium.SingleTileImageryProvider({
        ...
        for(var i =0;i<n ;i++){
            viewer.entities.add({
                position: Cesium.Cartesian3.fromDegrees(x[i], y[i]),
                point:{
                    pixelsize:10,
                },
                label:{
                    text: tempture[i].properties.NAME+": "+t[i],
                    font: '10px',
                }
            })
        }
