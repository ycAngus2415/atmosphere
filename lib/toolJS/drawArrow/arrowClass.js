...
var StraightArrow = function(viewer) {
	...
	this.pointImageUrl = "./data/img/glyphicons_242_google_maps.png";
...
StraightArrow.prototype = {
		...
		this.handler.setInputAction(function(evt) { //移动时绘制面
			...
			if ($this.positions.length >= 2) {
				if (!Cesium.defined($this.arrowEntity)) {
					...
					$this.positions.pop();
					$this.positions.push(cartesian);
					$this.viewer.scene.primitives.remove($this.arrowEntity);
				}
				$this.arrowEntity = $this.addArrow($this.positions);
			}
		}, Cesium.ScreenSpaceEventType.MOUSE_MOVE);
	},
	...
	showArrowOnMap: function(positions) {
		...
		// 			polygonHierarchy:new Cesium.CallbackProperty(update,false),
	...
	addArrow:function(positions){
		return this.viewer.scene.primitives.add(new Cesium.Primitive({
			geometryInstances: new Cesium.GeometryInstance({
				geometry: new Cesium.PolygonGeometry({
					polygonHierarchy: this.showArrowOnMap1(positions),
					vertexFormat: Cesium.EllipsoidSurfaceAppearance.VERTEX_FORMAT,
				})
			}),
	...
	showArrowOnMap1: function(positions) {
		var $this = this;
	...
	cartesianToLatlng: function(cartesian) {
		var latlng = Cesium.Cartographic.fromCartesian(cartesian);
...
export {StraightArrow,AttackArrow,PincerArrow}
