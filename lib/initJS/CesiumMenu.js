 ...
 * @Last Modified time: 2020-02-14 15:17:16
 */
...
import {
	meshVisual
} from '../toolJS/mesh/meshvisual.js';
	...
	constructor(viewer) {
		...
		var op = function() {
			...
			this.statFram = function() {
				...
				TweenMax.to($('#statframleftID'), 0.6, {
					x: "+=500",
					opacity: 1
				});
				TweenMax.to($('#statframrightID'), 0.6, {
					x: "-=500",
					opacity: 1
				});
				new TempStat('tempS');
			...
			this.ShowLimit = function() {
				...
				viewer.scene.camera.flyTo({
					destination: Cesium.Rectangle.fromDegrees(viewModel1.leftLon, viewModel1.leftLat, viewModel1.rightLon, viewModel1.rightLat),
				})
				Cesium.knockout.track(viewModel1);
				var toolbar1 = document.getElementById('limitOptionId');
				Cesium.knockout.applyBindings(viewModel1, toolbar1);
				Cesium.knockout.getObservable(viewModel1, 'leftLat').subscribe(
					function(value) {
						viewModel1.leftLat = value < viewModel1.rightLat ? value : viewModel1.rightLat - 0.000000001;
					}
				);
				Cesium.knockout.getObservable(viewModel1, 'rightLat').subscribe(
					function(value) {
						viewModel1.rightLat = value > viewModel1.leftLat ? value : viewModel1.leftLat + 0.000000001;
					}
				);
				Cesium.knockout.getObservable(viewModel1, 'leftLon').subscribe(
					function(value) {
						viewModel1.leftLon = value < viewModel1.rightLon ? value : viewModel1.rightLon - 0.000000001;
					}
				);
				Cesium.knockout.getObservable(viewModel1, 'rightLon').subscribe(
					function(value) {
						viewModel1.rightLon = value > viewModel1.leftLon ? value : viewModel1.leftLon + 0.000000001;
					}
				);

				function update() {
					let rec = Cesium.Rectangle.fromDegrees(viewModel1.leftLon, viewModel1.leftLat, viewModel1.rightLon, viewModel1.rightLat);
					viewer.scene.globe.cartographicLimitRectangle = rec;
					return rec;
				}
				...
				$("#closeLimit").click(function() {
					$("#limitOptionId").fadeOut();
		...
		$("#limitId").click(opts.ShowLimit);
		$("#meshVisualId").click(opts.ShowMesh)
		// $('#temp').click()
		...
		var toolbar = document.getElementById('insetImagery');
